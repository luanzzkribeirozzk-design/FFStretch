...
class MainActivity : AppCompatActivity() {
    ...
    // Botao ESTICAR: estica a tela E abre o FF automaticamente
    @SuppressLint("SetTextI18n")
    private fun stretchAndOpen() {
        ...
        // Esticar laterais horizontalmente:
        // Reduz o DPI para que a imagem fique mais larga visualmente
        // mantendo a resolucao fisica igual
        // Exemplo: 1080x2400 dpi=480 -> dpi=380 = imagem fica ~26% mais larga
        val stretchFactor = 1.0 + selectedPercent / 100.0
        val newDpi = (origDpi / stretchFactor).toInt()

        tvStatus.text = "Esticando tela..."
        // Muda o DPI para esticar horizontalmente (mesma logica do modo cachoeira)
...
Abrindo Free Fire..."
    ...
    private fun optimizeFF() {
        ...
        Thread {
            ...
            runOnUiThread {
                ...
                tvStatus.text = if (found) "Otimizado! FF pronto.\nRAM livre: ${free}MB"
                                else "Otimizado!\nRAM livre: ${free}MB"
                showToast("Pronto!")
            }
        }.start()
    ...
    private fun openFreefire() {
        ...
        for (pkg in ffPackages) {
            try {
                ...
                if (intent != null) {
                    ...
                    tvStatus.text = "Free Fire aberto!\nTela esticada +${selectedPercent}%"
                    return
                }
            } catch (_: Exception) {}
        }

        // Fallback: busca qualquer app com "freefire" ou "garena" no nome
        ...
        // Mostra todos os apps instalados para debug
        val ffApps = apps.filter {
            it.packageName.contains("free") || it.packageName.contains("garena")
        }.map { it.packageName }

        tvStatus.text = if (ffApps.isEmpty())
            "Free Fire nao encontrado!\nVerifique se esta instalado."
        else
    ...
    private fun updateStatus() {
        ...
        tvStatus.text = "Resolucao: ${m.widthPixels}x${m.heightPixels}\n" +
            if (foundPkg != null) "Free Fire: $foundPkg" else "Free Fire nao encontrado"
        updateShizukuStatus()
    }
